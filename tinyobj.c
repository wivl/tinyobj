...
tobj_model* tobj_load_model(const char *filename, bool has_texture) {
	...
	while (line_size >= 0) {
		...
		if (strncmp(line_buf, "v ", 2) == 0) {
			...
			face[0].x = strtof(ptr, &end); ptr = end+1;
			// printf("[DEV]f %ld/", l);
			face[0].y = strtof(ptr, &end); ptr = end+1;
			// printf("%ld/", l);
			face[0].z = strtof(ptr, &end); ptr = end+1;
			// printf("%ld ", l);


			face[1].x = strtof(ptr, &end); ptr = end+1;
			// printf("%ld/", l);
			face[1].y = strtof(ptr, &end); ptr = end+1;
			// printf("%ld/", l);
			face[1].z = strtof(ptr, &end); ptr = end+1;
			// printf("%ld ", l);


			face[2].x = strtof(ptr, &end); ptr = end+1;
			// printf("%ld/", l);
			face[2].y = strtof(ptr, &end); ptr = end+1;
			// printf("%ld/", l);
			face[2].z = strtof(ptr, &end); ptr = end+1;
			// printf("%ld\n", l);
